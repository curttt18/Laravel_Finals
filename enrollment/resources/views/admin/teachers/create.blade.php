...
@section('breadcrumb', $roleLabel . ' / Teachers / Create')
...
@section('content')
    <div class="card" style="max-width: 600px;">
        <div class="card-header">
            <h3 class="card-title"><i class="ri-user-star-line" style="margin-right: 8px; color: var(--c-coral);"></i>Add New Teacher</h3>
        </div>
        
        <form action="{{ route($prefix . '.teachers.store') }}" method="POST">
            @csrf
            
            <div class="form-group">
                <label for="teacher_name" class="form-label">Teacher Name</label>
                <input type="text" name="teacher_name" id="teacher_name" value="{{ old('teacher_name') }}" class="form-control" placeholder="Enter teacher's full name" required>
                @error('teacher_name')
            ...
            <div style="display: flex; justify-content: flex-end; gap: 12px; margin-top: 24px; padding-top: 20px; border-top: 1px solid var(--border);">
                ...
                <button type="submit" class="btn btn-primary">
                    <i class="ri-add-line"></i> Create Teacher
                </button>
            </div>
        </form>
    </div>
