...
@section('page-title', 'Edit Grade Record')
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <form action="{{ route('admin.grades.update', $grade) }}" method="POST">
                    ...
                    <div>
                        ...
                        <select name="student_id" id="student_id" class="w-full px-3 py-2 border border-gray-300 rounded-md" required>
                            @foreach($students as $student)
                                <option value="{{ $student->student_id }}" {{ $grade->student_id == $student->student_id ? 'selected' : '' }}>{{ $student->student_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="teacher_id" id="teacher_id" class="w-full px-3 py-2 border border-gray-300 rounded-md" required>
                            @foreach($teachers as $teacher)
                                <option value="{{ $teacher->teacher_id }}" {{ $grade->teacher_id == $teacher->teacher_id ? 'selected' : '' }}>{{ $teacher->teacher_name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="mb-4">
                    ...
                    <select name="academic_period" id="academic_period" class="w-full px-3 py-2 border border-gray-300 rounded-md" required>
                        @foreach(['Q1', 'Q2', 'Q3', 'Q4'] as $period)
                            <option value="{{ $period }}" {{ $grade->academic_period === $period ? 'selected' : '' }}>{{ $period }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                @foreach($skills as $skill)
                    <div class="mb-3">
                        ...
                        <select name="{{ $skill }}" id="{{ $skill }}" class="w-full px-3 py-2 border border-gray-300 rounded-md" required>
                            @foreach(['excellent', 'good', 'satisfactory', 'needs_improvement'] as $level)
                                <option value="{{ $level }}" {{ $grade->$skill === $level ? 'selected' : '' }}>{{ ucwords(str_replace('_', ' ', $level)) }}</option>
                            @endforeach
                        </select>
                    </div>
                @endforeach
                ...
                <div class="mb-6 mt-4">
                    ...
                    <textarea name="teacher_remarks" id="teacher_remarks" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-md">{{ $grade->teacher_remarks }}</textarea>
                </div>
                ...
                <div class="flex justify-end space-x-3">
                    ...
                    <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Update Grade Record</button>
                </div>
            </form>
        </div>
    </div>
