            ...
            <form action="{{ route('admin.payments.update', $payment) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <select name="fee_id" id="fee_id" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500" required>
                        @foreach($fees as $fee)
                            <option value="{{ $fee->fee_id }}" {{ old('fee_id', $payment->fee_id) == $fee->fee_id ? 'selected' : '' }}>{{ $fee->fee_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="number" step="0.01" name="payment_amount" id="payment_amount" value="{{ old('payment_amount', $payment->payment_amount) }}" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500" required>
                    </div>
                    <div>
                        <label for="payment_date" class="block text-sm font-medium text-gray-700 mb-1">Date</label>
                        <input type="date" name="payment_date" id="payment_date" value="{{ old('payment_date', $payment->payment_date->format('Y-m-d')) }}" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500" required>
                    </div>
                </div>
                <div class="mb-4">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="remarks" id="remarks" value="{{ old('remarks', $payment->remarks) }}" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500">
                </div>
