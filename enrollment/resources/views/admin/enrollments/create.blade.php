...
@section('breadcrumb', $roleLabel . ' / Enrollments / Create')
...
@section('content')
    <div class="card" style="max-width: 600px;">
        <div class="card-header">
            <h3 class="card-title"><i class="ri-file-add-line" style="margin-right: 8px; color: var(--c-coral);"></i>Add New Enrollment</h3>
        </div>
        
        <form action="{{ route($prefix . '.enrollments.store') }}" method="POST">
            @csrf
            ...
            <div class="form-group">
                ...
                <input type="text" name="school_year" id="school_year" value="{{ old('school_year', date('Y').'-'.(date('Y')+1)) }}" class="form-control" placeholder="e.g., 2024-2025" required>
                @error('school_year')
                    <p style="color: var(--danger); font-size: 0.85rem; margin-top: 6px;">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="date" name="enrollment_date" id="enrollment_date" value="{{ old('enrollment_date', date('Y-m-d')) }}" class="form-control" required>
            ...
            <div style="display: flex; justify-content: flex-end; gap: 12px; margin-top: 24px; padding-top: 20px; border-top: 1px solid var(--border);">
                ...
                <button type="submit" class="btn btn-primary">
                    <i class="ri-add-line"></i> Create Enrollment
                </button>
            </div>
        </form>
    </div>
